    ...
    e.fn.uploadFile = function (n) {
        ...
        function f(t, n, r) {
            ...
            r.on("drop", function (r) {
                ...
                if (!n.multiple && i.length > 1) {
                    if (n.showError) e("<div style='color:red;'>" + n.multiDragErrorStr + "</div>").appendTo(t.errorLog);
                    return
                }
        ...
        function h(t, n, r) {
            for (var i = 0; i < r.length; i++) {
                if (!p(n, t, r[i].name)) {
                    if (t.showError) e("<div style='color:red;'><b>" + r[i].name + "</b> " + t.extErrorStr + t.allowedTypes + "</div>").appendTo(n.errorLog);
                    continue
                }
                if (t.maxFileSize != -1 && r[i].size > t.maxFileSize) {
                    if (t.showError) e("<div style='color:red;'><b>" + r[i].name + "</b> " + t.sizeErrorStr + l(t.maxFileSize) + "</div>").appendTo(n.errorLog);
                    continue
                }
                if (t.maxFileCount != -1 && n.selectedFiles >= t.maxFileCount) {
                    if (t.showError) e("<div style='color:red;'><b>" + r[i].name + "</b> " + t.maxFileCountErrorStr + t.maxFileCount + "</div>").appendTo(n.errorLog);
                    continue
                }
                n.selectedFiles++;
                var s = t;
        ...
        function m(n, r, i, s) {
            ...
            f.change(function () {
                ...
                if (this.files) {
                    ...
                    a.push(f);
                    if (!p(n, i, f)) {
                        if (i.showError) e("<div style='color:red;'><b>" + f + "</b> " + i.extErrorStr + i.allowedTypes + "</div>").appendTo(n.errorLog);
                        return
                    }
                    l.push({
                        name: f,
                ...
                if (t.fileapi && t.formdata) {
                    ...
                    if (i.maxFileCount != -1 && n.selectedFiles + a.length > i.maxFileCount) {
                        if (i.showError) e("<div style='color:red;'><b>" + d + "</b> " + i.maxFileCountErrorStr + i.maxFileCount + "</div>").appendTo(n.errorLog);
                        return
                    }
